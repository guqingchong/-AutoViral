...
function rowToWork(row: Record<string, unknown>): DbWork {
  return {
    ...
    voice_id: (row.voice_id as string) || undefined,
    ...
    eval_mode: (row.eval_mode as string) || undefined,
    quality_mode: (row.quality_mode as string) || undefined,
    aspect: (row.aspect as string) || undefined,
    pipeline_version: (row.pipeline_version as number) ?? undefined,
    research_depth: (row.research_depth as string) || undefined,
...
export function createWork(work: DbWork, steps: DbPipelineStep[]): DbWork {
  ...
  const insert = db.prepare(
    `INSERT INTO works (id, title, type, content_category, content_form, video_source, video_search_query, status, platforms, evaluation_mode, topic_hint, topic_id, article_id, script_id, digital_human_id, voice_id, cli_session_id, account_id, eval_session_ids, eval_attempts, topic_category, emotion_type, hook_type, template_id, tags, estimated_cost, actual_cost, review_comment, asset_form, asset_source, asset_budget, dual_output, parent_work_id, auto_mode, purpose, explicit_params, eval_mode, quality_mode, aspect, pipeline_version, research_depth, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );
  ...
  const tx = db.transaction(() => {
    insert.run(
      ...
      work.eval_mode ?? null,
      work.quality_mode ?? null,
      work.aspect ?? null,
      work.pipeline_version ?? null,
      work.research_depth ?? null,
      work.created_at,
      work.updated_at
    );
...
export function updateWork(id: string, updates: Partial<DbWork>): DbWork | undefined {
  ...
  db.prepare(
    `UPDATE works SET
      ...
      topic_category = ?, emotion_type = ?, hook_type = ?, template_id = ?, tags = ?, estimated_cost = ?, actual_cost = ?, review_comment = ?, asset_form = ?, asset_source = ?, asset_budget = ?, dual_output = ?, auto_mode = ?, purpose = ?, explicit_params = ?, eval_mode = ?, quality_mode = ?, aspect = ?, updated_at = ?
     WHERE id = ?`
  ).run(
    work.title,
    work.type,
