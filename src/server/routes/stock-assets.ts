...
/** GET /api/stock-assets/search?q=... - search across Openverse + Pexels/Pixabay/Unsplash */
stockAssetRoutes.get("/search", async (c) => {
  ...
  try {
    const results = await searchStockAssets(query, { providers, perPage });
...
/** POST /api/stock-assets/download - download a stock asset into the library */
