...
export function buildMaterialSearchInstruction(work: { id: string; title: string; videoSearchQuery?: string }, isAutoMode: boolean): string {
  ...
  return [
    ...
    `搜索主题: "${query}"`,
    ``,
    `## 检索通道(两条都要用;key 由服务端持有,你不需要也不应该去找任何 API key)`,
    ...
    `3. **筛选与剔除**: 剔除水印/低清/题材不符项并记录剔除理由;保留候选每条写选用理由`,
    `4. **结构化留痕(强制)**: 写 assets/material-candidates.md,含查询组清单与命中情况、候选列表(路径/来源URL/时长/分辨率/授权/选用理由)、剔除记录——plan 阶段按路径直接引用`,
    ``,
    isAutoMode
