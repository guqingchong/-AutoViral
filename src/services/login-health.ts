 ...
 *
 * 2026-08-21 修复:画像目录必须走发布器同款 resolveProfileDir(platform, accountId)
 * —— Task 5 起登录/发布都在 per-account 画像(browser-profiles/<platform>/<accountId>),
 * 此前本函数死查旧单级目录 browser-profiles/channels,扫码成功也报"需重新登录"。
 */
async function verifyChannels(getCred: CredGetter = platformCred("channels"), accountId?: string): Promise<PlatformHealth> {
  ...
  const profileDir = resolveProfileDir(platform, accountId);
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      if (signedIn > 0) {
        // 画像内 cookie 可能已刷新,顺便回写凭证库保持同步(旧表兜底 + 账号维度)
        const cookies = await context.cookies();
        const serialized = JSON.stringify(cookies);
        const { setCredential } = await import("../db/platform-credentials-repo.js");
        setCredential(platform, "session_cookie", serialized);
        if (accountId) {
          const { setAccountCredential } = await import("../db/account-credentials-repo.js");
          setAccountCredential(accountId, "session_cookie", serialized);
        }
  ...
  try {
    ...
    for (const account of accounts) {
      ...
      if (account.platform in RPA_CHECKS && browser) {
        ...
        h = await verifyChannels(getCred, account.id);
