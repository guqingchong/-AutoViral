...
| 单行字数 | **硬约束 ≤15 个中文字符** | 部分 ffmpeg 构建的 libass 无 CJK 断词器，无空格中文长行**不会自动换行**，直接溢出画面左右边界（2026-08-16 实战事故）。caption_generate.py 已内置 `--max-chars 15`（默认开启，整句级时间戳条目会自动按字拆开保 karaoke 粒度），不要传更大的值绕过 |
