...
## 精品出片规范 — 强制执行(2026-08-14)

策划方案除了情绪钩子,还必须满足以下精品硬约束:
...
- **素材资产库（2026-08-14 新增)**：`curl "http://localhost:3271/api/assets/library?q=主题"` 检索已沉淀的素材(成片/片段/图表/快照卡)。**有可复用素材时优先复用**(使用频次高的=验证过的),并在分镜表中标注复用素材的 path,避免重复生成;复用历史素材还能强化系列感
...
> **进阶：** 除以下基础构图规则外，`modules/visual-aesthetics.md` 第三章提供了完整的13种构图模式（SAMP-Net分类）及其适用场景、Prompt关键词和竖屏/方形构图差异对照表。需要精细构图指导时请加载该模块。
...
> **进阶：** 如需更详细的配色模型（互补色/类似色/三角色等）、2026平台配色趋势、以及按内容类型选色的完整矩阵，请加载 `modules/visual-aesthetics.md` 第二章"色彩理论进阶"。
...
| 镜号 | 场景描述 | 首帧描述 (Image Prompt) | 时长 | 旁白/字幕 | 镜头运动 | 镜头类型 (shotType) | 音轨意图 (audioIntent) | 末帧描述（可选） |
|------|---------|----------------------|------|----------|---------|--------------------|----------------------|-----------------|
| 01   | [中文场景描述] | [详细英文提示词，包含角色参考、环境、光线、镜头角度、风格关键词] | 3s | [旁白或画面文字] | [镜头运动：固定/推/拉/摇/等] | narration | 无（走 TTS） | [如需平滑过渡] |
| 02   | ... | ... | ... | ... | ... | ... | ... | ... |

**镜头类型（shotType）四选一，AI 生成视频时必须标注**（本地 H3 生成依赖此字段做音画同生路由）：

| shotType | 适用镜头 | 音轨意图 (audioIntent) 写法 | 生成方式 |
|----------|---------|---------------------------|---------|
| `broll` | 氛围/空镜（环境渲染、过渡镜头） | 环境音描述，如「雨声+街道底噪」 | H3 图生视频，原生音轨压低作垫层 |
| `dialogue` | 对白/播报（人物开口说话） | **完整对白文本**，如「主持人说：城投转型正在加速」 | H3 **文生视频**（不用首帧，避免约束口型），跳 TTS |
| `narration` | 解说配图（配合 TTS 旁白的画面） | 无（走 TTS） | H3 图生视频，丢弃原生音轨 |
| `hero` | 精品镜头（海报级画面） | 无（走 TTS） | premium 档走云端 Seedance/即梦；eco 档也用 H3 |

规则：
- `dialogue` 镜头的 audioIntent 必须是**逐字对白文本**——H3 按它生成口型和语音，合成阶段按它对齐字幕（不依赖 ASR）
- `dialogue` 镜头不生成首帧图片，直接文生视频
- 一个作品中 `dialogue` 镜头不宜超过 2 个（音画同生质量不如 TTS 稳定，用在最有表现力的位置）
...
- **封面设计原则:** 主标题占图片宽度40-50%，位于上1/3；主体占50-60%画面；留白10-15%。参考小红书6大爆款封面规律（质感为王、强反差对比、图文结合等）。详见 `modules/visual-aesthetics.md` 第一章。
...
> **无人值守模式（批量/自动流水线）：会话 prompt 声明"自动化模式/自主拍板"时，步骤 3 和 5 的用户确认环节跳过——方向与方案均由你按本 skill 及各模块的推荐标准自行拍板，直接产出完整方案并推进流水线，禁止提问或等待。**

1. **获取上下文：** 读取作品详情、调研报告（如已完成调研步骤）、共享素材和用户记忆档案。
2. **阅读平台参考资料：** 根据目标平台，从 `references/` 目录中阅读对应的参考文件。
3. **确认方向：** 简要总结计划的内容方向，请用户确认后再创建完整方案（无人值守模式：自行拍板，跳过）。
4. **创建方案：** 按照上述格式输出完整方案，融入参考文件中的平台特定细节。
5. **与用户对审：** 展示方案并征求反馈。反复修改直到用户确认（无人值守模式：对照评审标准 content-evaluator/criteria/plan.md 自审一遍即过）。
6. **保存方案：** 更新作品的流水线状态。
